...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "vibebot-update-preflight-"));
    ...
    const doctorStep = await runStep(
      step(
        "vibebot doctor",
        managerScriptArgs(manager, "vibebot", ["doctor", "--non-interactive"]),
        gitRoot,
        { VIBEBOT_UPDATE_IN_PROGRESS: "1" },
      ),
    );
