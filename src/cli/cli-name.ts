...
export const DEFAULT_CLI_NAME = "vibebot";
export const LEGACY_CLI_NAME = "moltbot";

const KNOWN_CLI_NAMES = new Set([DEFAULT_CLI_NAME, LEGACY_CLI_NAME, "clawdbot"]);
const CLI_PREFIX_RE = /^(?:((?:pnpm|npm|bunx|npx)\s+))?(vibebot|moltbot|clawdbot)\b/;
...
export function resolveCliName(
  ...
  const override =
    env.VIBEBOT_CLI_NAME?.trim() || env.MOLTBOT_CLI_NAME?.trim() || env.CLAWDBOT_CLI_NAME?.trim();
