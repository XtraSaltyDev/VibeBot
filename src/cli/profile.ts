...
function resolveProfileStateDir(profile: string, homedir: () => string): string {
  ...
  return path.join(homedir(), `.vibebot${suffix}`);
}
...
export function applyCliProfileEnv(params: {
  ...
  env.VIBEBOT_PROFILE = profile;

  const stateDir =
    env.VIBEBOT_STATE_DIR?.trim() ||
    env.CLAWDBOT_STATE_DIR?.trim() ||
    resolveProfileStateDir(profile, homedir);
  if (!env.VIBEBOT_STATE_DIR?.trim() && !env.CLAWDBOT_STATE_DIR?.trim()) {
    env.VIBEBOT_STATE_DIR = stateDir;
  }

  if (!env.VIBEBOT_CONFIG_PATH?.trim() && !env.CLAWDBOT_CONFIG_PATH?.trim()) {
    env.VIBEBOT_CONFIG_PATH = path.join(stateDir, "vibebot.json");
  }

  if (
    profile === "dev" &&
    !env.VIBEBOT_GATEWAY_PORT?.trim() &&
    !env.CLAWDBOT_GATEWAY_PORT?.trim()
  ) {
    env.VIBEBOT_GATEWAY_PORT = "19001";
  }
}
